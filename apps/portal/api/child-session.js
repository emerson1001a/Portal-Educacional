...
export default async function handler(req, res) {
  ...
  if (!token) return json(res, 400, { ok: false, message: "Token infantil nao informado." });
  ...
  if (accessError) return json(res, 500, { ok: false, message: accessError.message });
  if (!access) return json(res, 401, { ok: false, message: "Acesso infantil invalido." });
  if (access.revoked_at) return json(res, 401, { ok: false, message: "Acesso infantil revogado." });
  if (new Date(access.expires_at).getTime() <= Date.now()) {
    return json(res, 401, { ok: false, message: "Acesso infantil expirado." });
  }
